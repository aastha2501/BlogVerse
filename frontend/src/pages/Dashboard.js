...
export default function Dashboard() {
  ...
  //only run if we have the user id means based on the dependency it will run
  useEffect(() => {
    if (userId) {
  ...
  return (
    ...
    >
      {userBlogs.length === 0 && (
        <h3>No blog available with the user: {user?.user?.name}</h3>
      )}
                    ...
                    >
                      ...
                      <Link to={`/editBlog/${item._id}`}>
                        <MDBIcon
                          fas
                          icon="edit"
                          style={{ color: "#7F167F", marginLeft: "10px" }}
                          size="lg"
                        />
                      </Link>
                    </div>
